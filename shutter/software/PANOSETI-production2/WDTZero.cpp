 ...
 * 
 * 06/17/2020 Franklin Antonio has installed the patch described in
 ...
 * to fix the syncrhonization problem that caused slowdown of the main loop.  It kept us from operating the stepping motor at even moderate
 * RPM while using microstepping.
 * 
...
void WDTZero::clear() {
//  WDT->CLEAR.reg = WDT_CLEAR_CLEAR_KEY;      // Clear WTD bit                                //FA
//  while(WDT->STATUS.bit.SYNCBUSY);                                                           //FA
    if (!WDT->STATUS.bit.SYNCBUSY) {           // Check if the WDT registers are synchronized  //FA
        REG_WDT_CLEAR = WDT_CLEAR_CLEAR_KEY;   // Clear the watchdog timer                     //FA
        }
