...
function main($run, $fname, $min, $max, $nframes) {
    $cmd = sprintf('php ../analysis/pipe_images.php derived/%s/%s/images.bin %d %d %d | ffmpeg -y -f rawvideo -pix_fmt argb -s 128x128 -r 25 -i - -pix_fmt yuv420p -c:v libx264 -movflags +faststart -vf scale=512:512 derived/%s/%s/images_%d_%d_%d.mp4',
        $run, $fname, $min, $max, $nframes, $run, $fname, $min, $max, $nframes
    );
    system($cmd);
