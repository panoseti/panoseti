...
//
function show_pixel_dir($vol, $run, $analysis_dir, $module_dir, $pixel_dir) {
    $dirpath = "$vol/analysis/$run/img_pulse/$analysis_dir/$module_dir/$pixel_dir";
    ...
    $spath = "$vol/analysis/$run/img_pulse/$analysis_dir/summary.json";
    ...
    for ($i=0; $i<$nlevels; $i++) {
        $x1 = 1<<$i;
        $path = "$vol/analysis/$run/img_pulse/$analysis_dir/$module_dir/$pixel_dir/thresh_$i.sorted";
        [$x, $n] = max_nsigma($path);
        if ($n) {
            $x2 = "<a href=img_pulse_show.php?action=list&vol=$vol&run=$run&analysis=$analysis_dir&module=$module_dir&pixel=$pixel_dir&level=$i>$n</a>";
            $x3 = $x;
        } else {
            $x2 = 0;
            $x3 = '---';
        ...
        if ($pixel_dir == 'all_pixels') {
            ...
            $path = "$vol/analysis/$run/img_pulse/$analysis_dir/$module_dir/$pixel_dir/all_$i";
            if (file_exists($path)) {
                $x4 = "<a href=img_pulse_show.php?action=graph&vol=$vol&run=$run&analysis=$analysis_dir&module=$module_dir&pixel=$pixel_dir&level=$i&start=0&n=1000>View graph</a>";
            } else {
                $x4 = '---';
            }
...
function show_list($vol, $run, $analysis_dir, $module_dir, $pixel_dir, $level) {
    $fname = "$vol/analysis/$run/img_pulse/$analysis_dir/$module_dir/$pixel_dir/thresh_$level.sorted";
    $lines = file($fname);
    page_head("Pulses");
