...
class TestParseName:
    ...
    def test_hk_file_parses_correctly(self) -> None:
...
class TestPffTypeHelpers:
    ...
    def test_is_pff_dir_true(self, name) -> None:
    ...
    def test_is_pff_dir_false(self, name) -> None:
...
class TestRunDirName:
    ...
    def test_different_obs_names(self) -> None:
        a = pff.run_dir_name("palomar", "sci")
        b = pff.run_dir_name("lick", "sci")
        assert "palomar" in a
        assert "lick" in b

    def test_different_run_types(self) -> None:
...
class TestWrToUnix:
    ...
    def test_d_zero_no_nsec(self) -> None:
    ...
    def test_d_zero_with_nsec(self) -> None:
    ...
    def test_d_one_returns_tv_sec_minus_one(self) -> None:
    ...
    def test_d_1023_returns_tv_sec_plus_one(self) -> None:
    ...
    def test_d_zero_with_zero_nsec_boundary(self) -> None:
        """Boundary: pkt_nsec=0, synchronized."""
        tv = self.TV_SEC
        tai = self._pkt_tai(tv, 0)
        assert pff.wr_to_unix(tai, 0, tv) == float(tv)

    def test_d_zero_with_max_nsec(self) -> None:
    ...
    def test_desync_raises_exception(self) -> None:
    ...
    def test_ignore_desync_returns_approximation(self) -> None:
    ...
    def test_different_tv_sec_values(self) -> None:
...
class TestImageRoundTrip:
    ...
    def test_round_trip_8bit(self) -> None:
    ...
    def test_image_block_starts_with_star(self) -> None:
    ...
    def test_read_image_bad_type_code_raises(self) -> None:
    ...
    def test_write_image_bad_params_raises(self) -> None:
    ...
    def test_image_all_zeros(self) -> None:
        img = [0] * 1024
        buf = io.BytesIO()
        pff.write_image_1D(buf, img, 32, 2)
        buf.seek(0)
    ...
    def test_image_max_values_16bit(self) -> None:
        img = [65535] * 1024
        buf = io.BytesIO()
        pff.write_image_1D(buf, img, 32, 2)
        buf.seek(0)
...
class TestReadJson:
    ...
    def test_reads_simple_header(self) -> None:
    ...
    def test_reads_nested_header(self) -> None:
    ...
    def test_bad_first_byte_raises(self) -> None:
        buf = io.BytesIO(b'X{"key": "val"}\n\n')
        with pytest.raises(Exception, match=r"read_json\(\): expected \{"):
            pff.read_json(buf)

    def test_full_pff_frame_roundtrip(self) -> None:
...
class TestImgHeaderTime:
    ...
    def test_nested_quabo0_format(self) -> None:
    ...
    def test_flat_ph256_format(self) -> None:
        """ph256: flat header without quabo_0 key."""
        h = {"pkt_tai": self._d0_tai(self.TV_SEC), "pkt_nsec": 0, "tv_sec": self.TV_SEC}
        assert pff.img_header_time(h) == float(self.TV_SEC)

    def test_nested_with_nsec(self) -> None:
...
class TestImgInfo:
    ...
    def test_basic_returns_four_tuple(self, pff_file_factory) -> None:
        f = pff_file_factory(n_frames=3, tv_sec_start=1_000_000)
        result = pff.img_info(f, self.BYTES_PER_IMAGE)
        assert len(result) == 4

    def test_nframes_correct(self, pff_file_factory) -> None:
        for n in [1, 3, 5]:
    ...
    def test_first_and_last_t(self, pff_file_factory) -> None:
        f = pff_file_factory(n_frames=3, tv_sec_start=1_000_000)
    ...
    def test_frame_size_formula(self, pff_file_factory) -> None:
    ...
    def test_skips_leading_zero_timestamp_frames(self, pff_file_factory) -> None:
    ...
    def test_all_zero_timestamps_raises(self, pff_file_factory) -> None:
    ...
    def test_flat_ph256_header(self, pff_file_factory) -> None:
...
class TestImgFrameTime:
    ...
    def test_frame_0_matches_first_t(self, pff_file_factory) -> None:
    ...
    def test_last_frame_matches_last_t(self, pff_file_factory) -> None:
        f = pff_file_factory(n_frames=5, tv_sec_start=2_000_000)
    ...
    def test_mid_frame(self, pff_file_factory) -> None:
...
class TestTimeSeek:
    ...
    def test_t_before_first_seeks_to_start(self, pff_file_factory) -> None:
        f = pff_file_factory(n_frames=5, tv_sec_start=1_000_000)
    ...
    def test_t_after_last_seeks_to_last_frame(self, pff_file_factory) -> None:
        f = pff_file_factory(n_frames=5, tv_sec_start=1_000_000)
        frame_size, nframes, _, last_t = pff.img_info(f, self.BYTES_PER_IMAGE)
        f.seek(0)
        pff.time_seek(f, frame_time=1.0, bytes_per_image=self.BYTES_PER_IMAGE,
    ...
    def test_t_exact_middle_seeks_near_that_frame(self, pff_file_factory) -> None:
...
class TestSkipImage:
    def test_skip_advances_past_image_block(self) -> None:
    ...
    def test_skip_then_read_next_header(self, pff_file_factory) -> None:
