...
class PsetiContainer:
    ...
    def __init__(
        ...
        *,
        ...
        local_uid = os.environ.get("LOCAL_UID")
        local_gid = os.environ.get("LOCAL_GID")
        if local_uid and local_gid:
            self._container.with_env("LOCAL_UID", local_uid)
            self._container.with_env("LOCAL_GID", local_gid)
