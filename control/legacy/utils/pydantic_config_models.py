...
class DaqNodeValidator(BaseStrictModel):
    # extra='ignore': the modernized control stack (test-pseti branch) adds
    # fields here (docker_context, modules, grpc_port, ...) that this branch
    # has no use for. Tolerating them lets both branches read the exact same
    # config files with zero drift, instead of maintaining a second,
    # hand-synced config directory just for this branch.
...
class DaqConfigValidator(BaseStrictModel):
    # extra='ignore': same rationale as DaqNodeValidator above -- the
    # modernized branch adds root-level fields here too (e.g.
    # daq_node_module_limit).
    model_config = ConfigDict(extra='ignore')
...
class PortForwarding(BaseStrictModel):
    # extra='ignore': same rationale as DaqNodeValidator above -- the
    # modernized branch adds a grpc_port field here for gateway-forwarded
    # gRPC traffic, which this branch has no use for.
    model_config = ConfigDict(extra='ignore')
