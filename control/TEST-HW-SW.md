...
We use Docker Compose `profiles` and the `DOCKER_HOST` SSH transport to maintain physical network realism without the complexity of Docker Swarm.

- **Headnode Profile:** Deployed locally on the head node.
- **DAQnode Profile:** Deployed to the remote DAQ node via `DOCKER_HOST=ssh://user@ip`.

### Zero-Overhead UDP Capture
The DAQ node container uses `network_mode: "host"`. This ensures the Hashpipe process has direct access to the physical NIC, bypassing the Docker bridge and guaranteeing zero-overhead capture of high-bandwidth UDP streams from the Quabos.

## 🚀 CLI Usage

The `pseti` (or `qa.py`) tool has been extended with a `test-hw` command group.
...
### 4. Run Tests
Execute the HW-SW test suite (Placeholder).
```bash
pseti test-hw run
```
...
- `daq_config.json`: Configures `/mnt/panoseti/test-hw/data/` as the primary SSD storage path.
