...
class Fleet:
    ...
    def start(self) -> None:
        ...
        for spec in self.specs:
            ...
            # Isolated volume per container
            # Using temp directory on host mapped to /data in container
            # In testcontainers, volumes are specified in kwargs
            # Actually with_volume_mapping is better if available in modern testcontainers
            # Or pass to kwargs['volumes']
    ...
    def wait_healthy(self, timeout: float = 60.0) -> None:
        ...
        # Testcontainers handles most of this via wait_for but we can be explicit
        for container in self._containers:
            # We don't map ports to host in this dynamic fleet usually, 
            # we rely on container-to-container networking.
            # But for testing from 'master' thread, we might need mapped ports.
            pass
...
def make_fleet(n: int, **kwargs: Any) -> Fleet:
    ...
    # Note: IPs are managed by Docker Network, we use Names as IPs in configs.
    specs = [
        DaqnodeSpec(
