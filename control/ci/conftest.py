...
def pytest_configure(config: Any) -> None:
    ...
    os.environ["PANOSETI_CONFIG_DIR"] = str(PanoPathsTest.integration_configs("direct"))

    # 2. Route state to isolated test directories
    os.environ["PANOSETI_TMP_DIR"] = str(PanoPathsTest.test_state_root() / "tmp")
    os.environ["PANOSETI_LOGS_DIR"] = str(PanoPathsTest.test_state_root() / "logs")
    os.environ["PANOSETI_QUABOS_DIR"] = str(PanoPathsTest.test_state_root() / "quabos")

    # 3. Ensure directories exist
    os.makedirs(os.environ["PANOSETI_TMP_DIR"], exist_ok=True)
    os.makedirs(os.environ["PANOSETI_LOGS_DIR"], exist_ok=True)
    os.makedirs(os.environ["PANOSETI_QUABOS_DIR"], exist_ok=True)
