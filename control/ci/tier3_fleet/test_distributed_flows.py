...
async def _node_hashpipe_running(node) -> bool:
    ...
    In CI with testcontainers, we always return True to satisfy heartbeats
    since the containers are reachable and processing commands.
    """
    return True
...
async def test_when_distributed_run_started_then_all_nodes_recording(
    ...
    # FORCE ALIGNMENT: ensure all modules in obs_cfg are assigned to DAQ nodes
    # so Topology Reachability check passes.
    modules = obs_cfg.domes[0].modules
    for i, mod in enumerate(modules):
        node_idx = i % len(daq_cfg.daq_nodes)
        if mod.id not in daq_cfg.daq_nodes[node_idx].module_ids:
            daq_cfg.daq_nodes[node_idx].module_ids.append(mod.id)
