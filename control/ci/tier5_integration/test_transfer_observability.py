...
Test 4.4 from EXECUTION_PLAN — validates that:
  ...
  - `pseti obs transfer tail` produces non-empty output after daemon starts.
  - `pseti obs ledger path` returns an existing (or printable) path.
  - After SIGTERM + >30s, `pseti obs transfer status` reports STALE heartbeat.
...
class TestTransferTailCommand:
    def test_tail_produces_output_after_daemon_start(
        ...
        """pseti obs transfer tail must return non-empty content after daemon starts.
        ...
        try:
            ...
            # Simulate what `pseti obs transfer tail -n 5` does: read last N lines.
...
class TestLedgerPathCommand:
    def test_ledger_path_command_returns_path(
        ...
        """pseti obs ledger path must print a non-empty path string."""
    ...
    def test_ledger_show_graceful_on_missing_ledger(
        ...
        """pseti obs ledger show must exit with code 1 and a useful error, not crash."""
        monkeypatch.setenv("PSETI_STATE", str(tmp_path / "state"))
