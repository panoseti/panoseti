...
class ObservingRunInterface:
    ...
    def __init__(self, data_dir, run_dir):
        ...
        if "image" in self.data_config:
            ...
            self.img_size = self.img_bpp * 32 ** 2  # Bytes per image: 1024 pixels, each requiring self.img_bpp bytes
            self.image_frame_size = None  # Bytes per frame, including JSON header, imaging data, and delimiters

        self.has_pulse_height = False
        if "pulse_height" in self.data_config:
            self.has_pulse_height = True
            self.ph_bpp = 2 # Default value
            self.ph_frame_size = None  # Bytes per frame, including JSON header, data, and delimiters
            self.pe_threshold = self.data_config["pulse_height"]["pe_threshold"]

            self.any_trigger = None
            self.ph_size = self.ph_bpp * 256  # Bytes per PH image: [256 or 1024] pixels, each requiring self.img_bpp bytes
    ...
    def stack_frames(self, start_file_idx, start_frame_offset, module_id, delta_t=1, agg='mean', allow_partial_image=False):
        ...
        for i in range(start_file_idx, -1, -1):
            ...
            with open(fpath, 'rb') as fp:
                ...
                fp.seek(frame_offset * self.image_frame_size, os.SEEK_CUR)
                ...
                if i > 0:
                    next_file_size = module_image_pff_files[i - 1]['nframes'] * self.image_frame_size
                    curr_byte_offset = frame_step_size * self.image_frame_size - fp.tell()
                    frame_offset = int((next_file_size - curr_byte_offset) / self.image_frame_size)
    ...
    def module_file_time_seek(self, module_id, target_time):
        ...
        with open(fpath, 'rb') as fp:
            ...
            frame_offset = int(fp.tell() / self.image_frame_size)
    ...
    def index_pulse_height_pff_files(self):
        ...
        for fname in os.listdir(self.run_path):
            ...
            with open(fpath, 'rb') as f:
                self.ph_frame_size, attrs["nframes"], attrs["first_unix_t"], \
                    attrs["last_unix_t"] = pff.img_info(f, self.ph_size)
    ...
    def index_image_pff_files(self):
        ...
        for fname in os.listdir(self.run_path):
            ...
            attrs["seqno"] = int(parsed_name["seqno"])
            with open(fpath, 'rb') as f:
                self.image_frame_size, attrs["nframes"], attrs["first_unix_t"], \
    ...
    def frame_iterator(self, fp, step_size: int, frame_limit=None):
        ...
        """
        return PFFFrameIterator(fp, step_size, self.image_frame_size, self.img_bpp, frame_limit)
